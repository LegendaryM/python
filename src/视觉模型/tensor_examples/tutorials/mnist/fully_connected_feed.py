...
def placeholder_inputs(batch_size):
  ...
  These placeholders are used as inputs by the rest of the models building
...
def do_eval(sess,
  ...
  Args:
    sess: The session in which the models has been trained.
...
def run_training():
  ...
  with tf.Graph().as_default():
    ...
    # Build a Graph that computes predictions from the inference models.
    ...
    for step in xrange(FLAGS.max_steps):
      ...
      # Run one step of the models.  The return values are the activations
      ...
      # Save a checkpoint and evaluate the models periodically.
      if (step + 1) % 1000 == 0 or (step + 1) == FLAGS.max_steps:
        checkpoint_file = os.path.join(FLAGS.log_dir, 'models.ckpt')
